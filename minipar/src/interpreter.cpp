...
ValueWrapper Interpreter::evaluate(Expression *expr)
{
    ...
    // 3. Arrays (Array) - Correção principal para garantir inicialização
    else if (auto *array = dynamic_cast<Array *>(expr))
    {
        ...
        for (const auto &elem : array->getElements())
        {
            ...
            if (!elem_value.isInitialized()) // Verificação de inicialização
            {
    ...
    // 4. Acesso (Access) - para arrays ou strings
    ...
    // 6. Operações Relacionais (Relational)
    ...
    // 7. Operações Aritméticas (Arithmetic)
    ...
    else if (auto *unary = dynamic_cast<Unary *>(expr))
    {
        ...
        if (op == "-")
        {
            ...
            LOG_DEBUG("Interpreter: Erro, operador unário '-' aplicado a tipo não numérico");
        ...
        LOG_DEBUG("Interpreter: Erro, operador unário não suportado: " << op);
        throw RunTimeError("Operador unário não suportado: " + op);
    }

    // 9. Operações Lógicas (Logical)
...
{
    if (!value.isInitialized()) // Verificação aprimorada usando isInitialized
    {
...
void Interpreter::execute_stmt(Node *stmt)
{
    ...
    if (auto *assign = dynamic_cast<Assign *>(stmt))
    {
        ...
        if (auto *id = dynamic_cast<ID *>(left))
        {
            ...
            {
                // Se a variável já é um array, atualiza os elementos sem sobrescrever o tamanho
            ...
            {
                // Caso seja uma variável nova ou não-array, sobrescreve normalmente
