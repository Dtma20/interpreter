...
std::unique_ptr<Node> Parser::stmt() {
    if (lookahead.getTag() == "ID") {
        ...
        match("FOR");
        if (!match("LPAREN")) {
            throw SyntaxError(lineno, "Esperado '(' após 'for' em lugar de " + lookahead.getValue());
        }
        skipWhitespace();
        // Processa a inicialização (ex.: i: number = 0)
        std::unique_ptr<Node> init;
        if (lookahead.getTag() == "ID") {
            auto var = local();  // Consome 'i'
            skipWhitespace();
            if (!match("COLON")) {
                throw SyntaxError(lineno, "Esperado ':' após identificador na inicialização do 'for' em lugar de " + lookahead.getValue());
            }
            skipWhitespace();
            std::string type = lookahead.getValue();
            if (!match("TYPE")) {
            ...
            skipWhitespace();
            if (!match("ASSIGN")) {
                throw SyntaxError(lineno, "Esperado '=' após tipo na inicialização do 'for' em lugar de " + lookahead.getValue());
            }
            skipWhitespace();
            auto value = arithmetic();
            init = std::make_unique<Assign>(std::move(var), std::move(value));
        }
        skipWhitespace();
        if (!match(";")) {
            throw SyntaxError(lineno, "Esperado ';' após inicialização do 'for' em lugar de " + lookahead.getValue());
        }
        skipWhitespace();

        // Processa a condição (ex.: i < 5)
        std::unique_ptr<Expression> condition;
        if (lookahead.getTag() != ";") {
            condition = disjunction();
        } else {
            condition = std::make_unique<Constant>("BOOL", Token("TRUE", "true"));
        }
        skipWhitespace();
        if (!match(";")) {
            throw SyntaxError(lineno, "Esperado ';' após condição do 'for' em lugar de " + lookahead.getValue());
        }
        skipWhitespace();
        ...
        if (lookahead.getTag() == "ID") {
            ...
            skipWhitespace();
            if (!match("ASSIGN")) {
                throw SyntaxError(lineno, "Esperado '=' no incremento do 'for' em lugar de " + lookahead.getValue());
            }
            skipWhitespace();
            auto value = arithmetic();
            increment = std::make_unique<Assign>(std::move(var), std::move(value));
        }
        skipWhitespace();
        if (!match("RPAREN")) {
            throw SyntaxError(lineno, "Esperado ')' após incremento do 'for' em lugar de " + lookahead.getValue());
        }
        skipWhitespace();
        if (!match("LBRACE")) {
            throw SyntaxError(lineno, "Esperado '{' após 'for' em lugar de " + lookahead.getValue());
        ...
        Body outer_body;
        if (init) {
            outer_body.push_back(std::move(init));
        }
        outer_body.push_back(std::move(while_stmt));
        return std::make_unique<Seq>(std::make_unique<Body>(std::move(outer_body)));
    }
